...
$$('[data-modal]').forEach(b=>b.addEventListener('click',()=>{mobile?.classList.remove('open');openModal(b.dataset.modal)}));$$('[data-close]').forEach(b=>b.addEventListener('click',()=>closeModal(b.closest('.modal'))));$$('.modal-backdrop').forEach(m=>m.addEventListener('click',e=>{if(e.target===m)closeModal(m.closest('.modal'))}));document.addEventListener('keydown',e=>{if(e.key==='Escape'){mobile?.classList.remove('open');$$('.modal.open').forEach(closeModal)}});
$$('[data-switch]').forEach(b=>b.addEventListener('click',()=>{closeModal(b.closest('.modal'));openModal(b.dataset.switch)}));
